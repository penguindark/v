...
fn (mut g Gen) sqlite3_stmt(node ast.SqlStmtLine, typ SqlType, db_expr ast.Expr) {
	...
	g.expr(db_expr)
	g.writeln(';')
	g.write('sqlite3_stmt* $g.sql_stmt_name = ${c.dbtype}__DB_init_stmt($db_name, _S("')
	...
	if node.kind == .insert {
		...
		for i, field in node.fields {
			...
			if field.typ == ast.string_type {
				...
				res := g.new_tmp_var()
				g.writeln('Array_sqlite__Row $res = sqlite__DB_exec($db_name, _S("SELECT last_insert_rowid()")).arg0;')
...
fn (mut g Gen) sqlite3_select_expr(node ast.SqlExpr, sub bool, line string, sql_typ SqlType) {
	...
	*/
	...
	// g.write('${dbtype}__DB_q_int(*(${dbtype}__DB*)${node.db_var_name}.data, _S("$sql_query')
	g.sql_stmt_name = g.new_tmp_var()
	db_name := g.new_tmp_var()
	g.writeln('\n\t// sql select')
	...
	g.write('string $stmt_name = _S("')
	g.write(g.get_base_sql_select_query(node, sql_typ))
	g.sql_expr_defaults(node, sql_typ)
	g.writeln('");')
	// g.write('sqlite3_stmt* $g.sql_stmt_name = ${dbtype}__DB_init_stmt(*(${dbtype}__DB*)${node.db_var_name}.data, _S("$sql_query')
...
fn (mut g Gen) mysql_select_expr(node ast.SqlExpr, sub bool, line string, typ SqlType) {
	...
	g.write('string $g.sql_stmt_name = _S("')
...
fn (mut g Gen) mysql_create_table(node ast.SqlStmtLine, typ SqlType, db_expr ast.Expr) {
	...
	tmp_str := 's_i(1, _MOV((Sid[]){{_S("Something went wrong: "), $si_s_code ,{.d_s=IError_str(err)}}}))'
...
fn (mut g Gen) mysql_bind(val string, typ ast.Type) {
	...
	g.sql_buf.write_string('$g.sql_stmt_name = string_replace($g.sql_stmt_name, _S("?$g.sql_i"), ')
	if sym != 'string' {
		mut num := false
		if sym != 'bool' {
			num = true
		...
		if sym == 'bool' {
			g.sql_buf.write_string('? _S("1") : _S("0")')
		}
		...
		g.sql_buf.write_string('string__plus(_S("\'"), string__plus(((string) $val), _S("\'")))')
	}
	g.sql_buf.writeln(');')
}
	...
	if node.kind == .insert {
		for i, field in node.fields {
			...
			if g.table.get_type_symbol(field.typ).kind == .struct_ {
				...
				res := g.new_tmp_var()
				g.writeln('Option_pg__Row $res = pg__DB_exec_one($db_name, _S("SELECT LASTVAL();"));')

				tmp_str := 's_i(1, _MOV((Sid[]){{_S0, $si_s_code ,{.d_s=IError_str(err)}}}))'
	...
	if node.is_count {
		...
		for i, field in node.fields {
			...
			if g.table.get_type_symbol(field.typ).kind == .struct_ {
				...
				g.writeln('${tmp}.$field.name = string__eq($fld, _S("0")) ? false : true;')
			} else {
